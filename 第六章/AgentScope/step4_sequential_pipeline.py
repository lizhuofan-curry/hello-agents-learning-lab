...
""",
    ...
    async with MsgHub(
        ...
        # MsgHub 管信息流  相当于微信群
        # sequential_pipeline 管执行流 相当于群主规定的发言顺序
        # sequential_pipeline() 返回的不是所有 Agent的 Msg 列表，而是最后一个 Agent返回的那一条 Msg
        # 特点就是前一个 Agent 的输出成为后一个 Agent 的输入
        response = await sequential_pipeline(
            [liubei, zhuge],
        )
